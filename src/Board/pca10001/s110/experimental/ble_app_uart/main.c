...
#define DEVICE_NAME                     "BLE_NAME"                                 /**< Name of device. Will be included in the advertising data. */
...
/*----------------------------------自定义添加---------------------------------------*/
//定义发送数据的缓冲区，这个功能是个转发数据的功能。所以程序上就不大改了，采用这样的策略去解决这个问题
//串口接收到数据后凑齐BLE_NUS_MAX_DATA_LEN就进行一次发送，同时每次接收到数据之后启动定时器（时间大于波特率）。
//当定时器超时后，即使没有收到足够BLE_NUS_MAX_DATA_LEN的数据也发送出去。
#define  UART_DELAY                 (2000U) 
#define  BLE_UART_REV_SIZE           2000
static uint8_t data_array[BLE_UART_REV_SIZE];                       //发送数据的缓冲区    
static volatile uint32_t index = 0;                                 //发送数据位置
...
void Time2_init(void)  //
{
    NRF_TIMER2->MODE        = TIMER_MODE_MODE_Timer;          
		...
		NRF_TIMER2->TASKS_COUNT = 0;
    NRF_TIMER2->PRESCALER   = 2;  
		NRF_TIMER2->TASKS_CLEAR = 1;
    NRF_TIMER2->CC[0]       = UART_DELAY;
    ...
    NVIC_DisableIRQ(TIMER2_IRQn);        //默认不打开
...
uint32_t  send_buffer(uint32_t tindex, uint8_t* pdata)
{
	int i = 0;
	uint32_t err_code;
	while (tindex > (BLE_NUS_MAX_DATA_LEN - 1))
		{
			err_code = ble_nus_send_string(&m_nus, pdata+i, BLE_NUS_MAX_DATA_LEN);
			if (err_code != NRF_ERROR_INVALID_STATE)
			{
					APP_ERROR_CHECK(err_code);
			}  
			tindex -= BLE_NUS_MAX_DATA_LEN;
			i += BLE_NUS_MAX_DATA_LEN;
		}
		if (tindex != 0)
		{
			err_code = ble_nus_send_string(&m_nus, pdata+i, tindex);
			if (err_code != NRF_ERROR_INVALID_STATE)
			{
					APP_ERROR_CHECK(err_code);
			}  
