...
/**@brief       Function for adding RX characteristic. 接收数据特性
 *
 * @param[in]   p_nus        Nordic UART Service structure.
...
static uint32_t rx_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    ble_gatts_char_md_t char_md;          //特性
    ble_gatts_attr_md_t cccd_md;          //属性单元cccd描述符
    ble_gatts_attr_t    attr_char_value;  //属性管理
    ble_uuid_t          ble_uuid;         //uuid
    ble_gatts_attr_md_t attr_md;          //属性单元
    ...
    cccd_md.vloc = BLE_GATTS_VLOC_STACK;      //设置变量存放于协议栈内存中，而不是用户内存
    
    memset(&char_md, 0, sizeof(char_md));     //初始化特性
    
    char_md.char_props.notify = 1;            //开启通知
    char_md.p_char_user_desc  = NULL;
    char_md.p_char_pf         = NULL;
    char_md.p_user_desc_md    = NULL;
    char_md.p_cccd_md         = &cccd_md;     //cccd特性
    char_md.p_sccd_md         = NULL;
    
    ble_uuid.type             = p_nus->uuid_type;               //添加uuud信息  TYPE=0
    ...
    memset(&attr_char_value, 0, sizeof(attr_char_value));//初始化属性管理

    attr_char_value.p_uuid    = &ble_uuid;      //前已经添加
    attr_char_value.p_attr_md = &attr_md;      //特性单元
    attr_char_value.init_len  = sizeof(uint8_t);
    attr_char_value.init_offs = 0;
    attr_char_value.max_len   = BLE_NUS_MAX_RX_CHAR_LEN;     //属性长度最大值
    
    return sd_ble_gatts_characteristic_add(p_nus->service_handle,  //服务句柄
                                           &char_md,               //添加的特性
                                           &attr_char_value,       //回去查下手册
                                           &p_nus->rx_handles);    //回去查下手册
    /**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
